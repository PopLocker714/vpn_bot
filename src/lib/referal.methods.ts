...
const get = async (userId: string) => {
    return await sqldb.query.$Referal.findFirst({ where: eq($Referal.user_id, userId) })
}

const setBy = async (user_id: string, referal_by: string) => {
    try {
        await sqldb.update($Referal).set({ referal_by })
...
const addRef = async (user_id: string, referal: string) => {
    try {
        const currentReferal = await sqldb.query.$Referal.findFirst({ where: eq($Referal.user_id, user_id) })
        if (!currentReferal) throw new Error('Not found referal')
        const isExist = currentReferal.referals.find((id) => id === referal)
        if (!isExist) {
            await sqldb.update($Referal)
