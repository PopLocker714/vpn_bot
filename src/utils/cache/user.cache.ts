...
export const setUserCache = async (user: CreateUserCommand.Response['response']) => {
    await sqlmem.insert($Users_c).values({ data: user, id: user.id, tg_id: user.telegramId, uuid: user.uuid }).onConflictDoUpdate({
        target: $Users_c.id,
        set: {
            data: user,
...
export const getUserCache = async ({ tg_id, uuid }: { tg_id?: number, uuid?: string }) => {
