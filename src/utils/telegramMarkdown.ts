    ...
    while (i < input.length) {
        ...
        if (input.startsWith("```", i) && !isEscapedAt(input, i)) {
            ...
            const closeIndex = findUnescaped(
                input,
                "```",
                headerEnd < input.length ? headerEnd + 1 : headerEnd,
            );
            if (closeIndex !== -1) {
                flushPlain(i);
                const header = input.slice(i, headerEnd);
                const hasNewline =
                    headerEnd < input.length && input[headerEnd] === "\n";
...
const applyPlaceholders = (input: string, values: unknown[]): string => {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        output = output
            .split(placeholder)
            .join(escapeMarkdownV2(stringifyTemplateValue(values[i])));
    }
    return output;
};
